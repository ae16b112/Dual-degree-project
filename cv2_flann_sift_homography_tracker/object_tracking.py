...
def find_homography(img, frame):
    ...
    if len(good_points)>MIN_MATCH_COUNT:
        ...
        cv2.line(frame, (0, y_medium), (2040,y_medium), (0, 255, 0), 2)
...
def object_tracking(img, video, tracker, tracker_type, selection_type):    
    ...
    while True:
        ...
        if ok:
            ...
            x_medium = int((x + x + w) / 2)
            y_medium = int((y + y + h) / 2)
            cv2.line(frame, (x_medium, 0), (x_medium, 840), (0, 255, 0), 2)
            cv2.line(frame, (0, y_medium), (2040, y_medium), (0, 255, 0), 2)
        else :
            # Tracking failure
...
if __name__ == '__main__' :
    ...
    object_tracking(img, video, tracker, tracker_type, selection_type)
